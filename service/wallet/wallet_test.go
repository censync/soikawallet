...
func TestWalletService_addAddress(t *testing.T) {
	...
	*/
	...
	for i := 0; i < 10; i++ {
		dPath, err := types.CreatePath(types.CoinType(types.Ethereum), 0, types.ChargeExternal, types.AddressIndex{Index: uint32(i), IsHardened: true})

		if err != nil {
			t.Fatal(err)
		}

		address, err := service.addAddress(dPath)
	...
	for i := 0; i < 10; i++ {
		dPath, err := types.CreatePath(types.CoinType(types.Tron), 0, types.ChargeExternal, types.AddressIndex{Index: uint32(i), IsHardened: true})
